...
template <class PhysicalQuantityPolicy, int TDim>
void createRender(const std::string & domain_filename,
...
{
	Image<TDim> img{ Domain< Space< PointI<TDim> > >() };

	// renderer initialized from files
	if(domain_filename == "")
	{
		TreeImageRenderer<TDim> renderer(radii_filename, vertices_filename, edges_filename);
	...
	{
		TreeImageRenderer<TDim> renderer(radii_filename, vertices_filename, edges_filename, domain_filename);

		// output_width is ignored if domain_filename was defined
		img = renderer.flowRender(output_width);
	}

	saveRender<TDim>(img, output_filename);
}


// Policy used to get the flow
// users are free to use any policy they see fit
...
int main(int argc, char *const *argv)
{
	...
	if (dimension == 2)
	{
		createRender<2>(domain_filename,
						radii_filename,
						edges_filename,
						vertices_filename,
						output_filename,
	...
	{
		createRender<3>(domain_filename,
						radii_filename,
						edges_filename,
						vertices_filename,
						output_filename,
